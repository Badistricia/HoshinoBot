...
black_word = ['今天我是什么少女', 'ba来一井','今日早报']  # 如果有不想触发的词可以填在这里
last_check = {}
...
async def query_models(bot, ev: CQEvent):
    ...
    # 从本地配置中获取模型列表
    available_models = config_manager.get_options()["api_providers"][provider].get("models", ["deepseek-v3"])
    
    if not available_models:
        available_models = ["deepseek-v3"]  # 如果没有模型列表，使用默认值
